...
/*
  -- Expected FormData fields: --
    ...
    'step_image_i': image blog. (represents the image associated to steps_metadata[i])
...
*/
async function handleCreateVisualRoute(
  request: Request,
  env: Env,
): Promise<Response> {
  ...
  try {
    ...
    if (!title?.trim() || !buildingId?.trim() || !stepsMetaRaw) {
      return jsonResponse(
        {
          error:
            "Missing required fields (title, building_id, status, steps_metadata).",
        },
        400,
      );
    }
    ...
    if (!isStepMetadataArray(stepsMeta)) {
      return jsonResponse(
        { error: "The provided visual steps are not valid." },
        400,
      );
    }
    ...
    for (let i = 0; i < stepsMeta.length; i++) {
      ...
      if (!file || !(file instanceof File) || file.type !== "image/webp") {
        return jsonResponse(
          { error: `Missing or invalid image file for step #${i + 1}` },
          400,
        );
      }
      ...
      if (file.size === 0) {
        return jsonResponse(
          { error: `Empty image file received for step #${i + 1}` },
          400,
        );
      }
    ...
    if (uploadedKeys.length > 0) {
      await Promise.allSettled(
        uploadedKeys.map((key) => env.BUCKET.delete(key)),
      );
    }

    const details = error instanceof Error ? error.message : String(error);
    return jsonResponse(
      { error: "Failed to create visual route", details },
      500,
    );
  }
}
